...
Eirik 当前在 `stormcliff`，因此该行动可用于查看返回 `skeld_village` 的三层 Preview。先检查 Preview，本阶段不需要立即调用 Commit。

## 14. 当前不负责

Web API v0.1 没有实现 Next.js / React UI、NPC Agent、NPC Memory、Relationship System、Dynamic Event、Multimodal、3D 或 Database。
